...
data = data[data['剩余天数(结束)'].apply(lambda x: x.split('天')[0]).astype(int) >= 2]
data = data[data['剩余天数(开始)'].apply(lambda x: x.split('天')[0]).astype(int) >= 2]
...
other_events = ['凤相竞舟跨服[4]', '玄武相竞舟跨服[4]', '仙园游宴(万妖灵塔)跨服[4]', '仙园游宴(山海套装)跨服[4]',
                    '仙戏灵龙跨服[4]', '虎相竞舟跨服[8]']
lei_chong_events = ['炼骨成圣', '仙戏灵龙', '封魔正法(五日连充)', '百炼金丹', '心花似悦', '御灵有术']
# chou_jiang_events = ['炼体秘藏_万毒混元身', '神阵鉴宝_涅槃珠', '炼体秘藏_须弥神象功', '天河仙会跨服[4]', '神阵鉴宝_梵花宝篮']
chou_jiang_events = ['炼体秘藏', '天河仙会', '神阵鉴宝']
